...
class ReviewSequentialDataset(data.Dataset):
    def __init__(self, user_seqs, mode='train', user_seqs_aug=None, review_seqs=None, flag_review=False):
        self.mode = mode
        self.max_seq_len = configs['model']['max_seq_len']
        self.user_history_lists = {user: seq for user,
                                   seq in zip(user_seqs["uid"], user_seqs["item_seq"])}
        if user_seqs_aug is not None:
            ...
            self.last_items = user_seqs_aug["item_id"]
            if flag_review:
                self.review_seqs = user_seqs_aug["review_seq"]
        else:
            self.uids = user_seqs["uid"]
            self.seqs = user_seqs["item_seq"]
            self.last_items = user_seqs["item_id"]
            if flag_review:
                self.review_seqs = review_seqs["review_seq"]
    ...
    def __getitem__(self, idx):
        seq_i = self.seqs[idx]
        uid = self.uids[idx]
        last_item = self.last_items[idx]
        item_seq = torch.LongTensor(self._pad_seq(seq_i))
        review_seq = self.review_seqs[idx]
    
        if self.mode == 'train' and configs['data'].get('neg_samp'):
            neg_item = torch.tensor(self.negs[idx], dtype=torch.long)
